...
// migratePublicChannels initializes the PublicChannels table before the triggers take over
// to keep it up-to-date.
//
// In a single server scenario, no new channels will be created between this migration and the
// corresponding trigger creation.
// In a multi-server scenario,
func migratePublicChannels(sqlStore SqlStore) error {
	transaction, err := sqlStore.GetMaster().Begin()
	if err != nil {
		return err
	}
...
func UpgradeDatabaseToVersion54(sqlStore SqlStore) {
	...
	if err := migratePublicChannels(sqlStore); err != nil {
